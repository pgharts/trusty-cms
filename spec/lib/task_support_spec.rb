...
describe TrustyCms::TaskSupport do
  describe "self.config_export" do
    ...
    it "should create YAML equal to TrustyCms::Config.to_hash" do
      described_class.config_export(@yaml_file)
      YAML.load_file(@yaml_file).should == TrustyCms::Config.to_hash.to_yaml
    end
  end
  describe "self.config_import" do
    ...
    it "should delete all TrustyCms::Config when the clear parameter is set to true" do
      ...
      TrustyCms::Config['testing_clear'].should be_nil
    end
    ...
    it "should update TrustyCms::Config with the settings from the given YAML" do
      TrustyCms::Config.delete_all
      described_class.config_import(@yaml_file)
      TrustyCms::Config.to_hash.should == YAML.load(YAML.load_file(@yaml_file))
    end
    it "should roll back if an invalid config setting is imported" do
      TrustyCms.config_definitions['defaults.page.status'].stub!(:select_from).and_return(['Draft'])
      TrustyCms::Config['defaults.page.status'] = "Draft"
      lambda{described_class.config_import(@bad_yaml_file)}.should_not raise_error
      TrustyCms::Config['defaults.page.status'].should == "Draft"
    end
  end
