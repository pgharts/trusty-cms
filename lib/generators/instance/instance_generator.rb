...
class Rails::Generator::Commands::Create
  def run_bundler(destination_root)
    ...
    command = %{"#{Gem.ruby}" -rubygems "#{Gem.bin_path('bundler',
                                                        'bundle')}" install --gemfile="#{File.join(
                                                          File.expand_path(destination_root), 'Gemfile'
                                                        )}"}
    begin
...
class InstanceGenerator < Rails::Generator::Base
  ...
  def manifest
    ...
    script_options     = { chmod: 0o755, shebang: options[:shebang] == DEFAULT_SHEBANG ? nil : options[:shebang] }
    dispatcher_options = { chmod: 0o755, shebang: options[:shebang] }
    ...
    record do |m|
      ...
      base_dirs = %w(config config/environments config/initializers db log script public vendor/plugins
                     vendor/extensions)
      ...
      files.each do |file|
        if File.directory?("#{root}/#{file}")
          m.directory file
        elsif file =~ %r{^script/}
          m.file radiant_root(file), file, script_options
        elsif file =~ %r{^public/dispatch}
